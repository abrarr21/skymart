...
const FeatureItem = ({ items, cardName }: FeatureProp) => {
    return (
        <div className="mb-4 rounded-3xl border border-white/20 bg-white p-6">
            {/* Header */}
            ...
            <div className="space-y-2">
                {items.map((item) => (
                    <NavLink
                        key={item.id}
                        to={"#"}
                        className="group flex items-center gap-3 rounded-2xl border border-ink/20 bg-white/3 p-3 transition-all duration-200 hover:border-volt/30 hover:bg-white/6"
                    >
                        {/* Image */}
                        ...
                        <div className="min-w-0 flex-1">
                            <p className="clamp-1 text-xs font-semibold text-ink/90">
                                {item?.title}
                            </p>
